...
public class CircleProgress extends View {
	...
	public CircleProgress(Context context, AttributeSet attr) {
		this(context, attr, 0);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (textIsDisplayable) {
			if (style == STROKE)
				canvas.drawText(text, center - textWidth / 2, center + textSize
						/ 2, paint);// 画出进度百分比
		} else {
			text = "配置";
			textWidth = paint.measureText(text); // 测量字体宽度，我们需要根据字体的宽度设置在圆环中间
			canvas.drawText(text, center - textWidth / 2,
					center + textSize / 2, paint);
		}
		...
		RectF oval = new RectF(center - radius, center - radius, center
				+ radius, center + radius);// 用于定义圆弧的形状和大小的界限
	...
	public synchronized void setProgress(int progress) {
		...
		if (progress <= max) {
			...
			// invalidate();
			postInvalidate();// 支持在其它线程更新view的视图
		}
	}
